...
import { ArrowLeft, Search, History, Heart, Bell, Loader2, CheckCircle, BookHeart } from "lucide-react";
...
function SearchContent() {
  ...
  // キーワード検索（タイトルの部分一致。ひらがな・カタカナ変換も併用）
  const executeSearch = async (query: string) => {
    ...
    try {
      ...
      const uniqueResults = searchResults.flat().filter((item: any) => {
          ...
          <div className="text-center py-12">
            ...
            {searchQuery.trim() && (
              <div className="mt-4 max-w-xs mx-auto">
                <p className="text-xs text-gray-400 mb-2">この本がまだ無い場合は</p>
                <Link
                  href={`/book-requests?title=${encodeURIComponent(searchQuery.trim())}`}
                  className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-xl font-semibold hover:bg-primary/90 transition-all shadow-md"
                >
