...
登録確認を **メールのリンクを踏む方式 → メールで届く6桁コードをアプリに入力する方式** に変える。
...
- `app/auth/signup/page.tsx` を6桁コード入力方式に変更
...
### 1. 確認メールのテンプレートを「6桁コード」に変更 ★これが無いと動かない

**Supabase Dashboard → Authentication → Email Templates → Confirm signup**

本文（Message body）を下記に差し替える。
**ポイント：リンク（`{{ .ConfirmationURL }}`）は消して、`{{ .Token }}`（6桁コード）を載せる。**
...
**Authentication → Providers → Email** の OTP expiry（コードの有効期限）を確認。
